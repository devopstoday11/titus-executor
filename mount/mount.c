...
void compose_final_options(const char *nfs_mount_hostname, char *final_options) {
...
void process_option(char* option, int fsfd) {
...
void do_fsconfigs(int fsfd, char *options) {
...
int fork_and_get_fsfd(long int pidfd) {
...
void switch_namespaces(int pidfd) {
	int ret;
...
void mount_and_move(int fsfd, const char* target, int pidfd) {
	int mfd = fsmount(fsfd, 0, MS_NODEV);
	if (mfd < 0) {
...
int main(int argc, char *argv[]) {
	...
	/* Now we can switch namespaces so we can lookup the ip for the addr */
	...
	mount_and_move(fsfd, target, pidfd);
